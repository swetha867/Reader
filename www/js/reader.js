...
//     console.log(JSON.stringify(pageList)); // Save the result
     ...
     book.on('book:pageChanged', function(location){
       ...
       currentPage.value = location.anchorPage;
	     ...
	     // db.transaction(function (tx) {
		    //  tx.executeSql('CREATE TABLE IF NOT EXISTS PageTimeTable (book, page, seconds)');
		    //  // if(arrayOfTimes.length==1){
		    //  //  tx.executeSql('INSERT INTO PageTimeTable VALUES (?,?,?)', [book.metadata.bookTitle, 0, 0 ]);
		    //  // }
		    //  if(arrayOfTimes.length==2){
			  //    tx.executeSql('INSERT INTO PageTimeTable VALUES (?,?,?)', [book.metadata.bookTitle, arrayOfPages[0], arrayOfTimes[1] - arrayOfTimes[0] ]);
		    //  }
	     // }, function (error) {
		    //  console.log('Transaction ERROR: ' + error.message);
	     // }, function () {
...
EPUBJS.Hooks.register("beforeChapterDisplay").selectword = function (callback, renderer, book) {
	...
	items.forEach(function (item) {
		...
		$(item).on('dblclick', function () {
			...
			var replacee = " " + t;
			var replacer = " <b class='hilite demo'>" + t + "</b>";
